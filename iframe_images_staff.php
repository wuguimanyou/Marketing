...
$staff_imgurl="";
if(!empty($_GET["staff_imgurl"])){
    $staff_imgurl=$configutil->splash_new($_GET["staff_imgurl"]);
}else{
	 if($customer_id>0){
	   $query2="select staff_imgurl from weixin_commonshops where isvalid=true and  customer_id=".$customer_id;
		$result2 = mysql_query($query2) or die('Query failed: ' . mysql_error());
		while ($row2 = mysql_fetch_object($result2)) {
		   $staff_imgurl=$row2->staff_imgurl;
		}
	}
}
$op="";
if(!empty($_GET["op"])){
   ...
   if($op=="del"){
	  ...
	  if($customer_id>0){
		  $query="update weixin_commonshops set staff_imgurl='' where customer_id=".$customer_id;
		  mysql_query($query);
	  }
	  $staff_imgurl="";
   }
...
<span class="upload_file">
	<div>
	   <form action="save_staff_imgurl.php?customer_id=<?php echo $customer_id_en; ?>" id="frm_img" enctype="multipart/form-data" method="post">
			<div class="up_input">
...
<div class="img" id="PicDetail">
        ...
        <?php if(!empty($staff_imgurl)){ ?>
		<div style="width:100%;height:120px;">
			 <a href="<?php echo $new_baseurl.$staff_imgurl; ?>" target="_blank">
			 <img style="width:100%;height:120px;" src="<?php echo $new_baseurl.$staff_imgurl; ?>"></a>
...
<script type="text/javascript">  
	...
	parent.setParentDefaultimgurl('<?php echo $staff_imgurl; ?>');
	
	
	function delImg(id){
	   url = "iframe_images_staff.php?op=del&i_id="+id+"&customer_id=<?php echo $customer_id_en; ?>";
