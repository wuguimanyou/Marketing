...
$query ="select isOpenPublicWelfare,nowprice_title,isOpenInstall,isOpenAgent,pro_card_level,shop_card_id from weixin_commonshops where isvalid=true and customer_id=".$customer_id;
...
<script type="text/javascript">
	...
	?>
	var search_type_id;
	var product_type_id	= '<?php echo $product_type_id;?>';
	var product_type_parent_id = '<?php echo $product_type_parent_id == -1 ? $product_type_id : $product_type_parent_id;?>';
	
	$(function(){
		$("#parent_type").change(function(){
			var sel_tchild = $("#child_type");
			sel_tchild.html("");
			var pid = $(this).val();
		...
		for(var i = 0 ; i < type_parent.length ; i++){
			...
			if(parseInt(product_type_parent_id) > 0 && product_type_parent_id == id){
				//isparent = true ; 
				sel_tparent.val(product_type_parent_id);
				sel_tparent.change();
			}
		}
		/*if(isparent == true){
			sel_tparent.val(product_type_id);
			sel_tparent.change();
		}*/
		if(product_type_parent_id != product_type_id){
			$("#child_type").val(product_type_id);
			$("#child_type").change();
		}
	});
</script>
  ...
  <form id="frm_product" class="r_con_form" method="post" action="save_product.php?customer_id=<?php echo $customer_id_en; ?>&pagenum=<?php echo $pagenum; ?>&adminuser_id=<?php echo $adminuser_id; ?>&owner_general=<?php echo $owner_general; ?>&orgin_adminuser_id=<?php echo $orgin_adminuser_id; ?>" enctype="multipart/form-data">
	<div class="WSY_data">
		 ...
		 <dl class="WSY_bulkbox w90px">
        	<dt>产品名称：</dt>
            <dd><input type="text" name="name" id="name" value="<?php echo $product_name;?>" readonly="readonly"></dd>
        </dl>
        
          
		  <dl class="WSY_bulkdl w90px" style="display:none">
