...
$welfare_images="";
if(!empty($_GET["welfare_images"])){
    $welfare_images=$configutil->splash_new($_GET["welfare_images"]);
}else{
	 if($customer_id>0){
...
if(!empty($_GET["op"])){
   ...
   if($op=="del"){
	  ...
	  if($customer_id>0){
		  $query="update weixin_commonshop_publicwelfare set backimg='' where customer_id=".$customer_id;
		  mysql_query($query);
	  }
	  $welfare_images="";
   }
...
<span class="upload_file">
	<div>
	   <form action="save_welfare_bg_imgurl.php?customer_id=<?php echo $customer_id_en; ?>" id="frm_img" enctype="multipart/form-data" method="post">
			<div class="up_input">
		...
		<div class="tips" style="font-size:12px;">上传1张图片，作为公益基金展示背景图。图片大小建议：<?php echo $n_width; ?>*<?php echo $n_height; ?>像素</div>
		<div class="clear"></div>
	</div>
</span>
...
<div class="img" id="PicDetail">
        ...
        <?php if(!empty($welfare_images)){ ?>
		<div style="width:100%;height:120px;">
			 <a href="<?php echo $new_baseurl.$welfare_images; ?>" target="_blank">
			 <img style="width:100%;height:120px;" src="<?php echo $new_baseurl.$welfare_images; ?>"></a>
...
<script type="text/javascript">  
	...
	parent.setWelfareDefaultimgurl('<?php echo $welfare_images; ?>');
	
	
	function delImg(id){
	   url = "iframe_images_welfare.php?op=del&i_id="+id+"&customer_id=<?php echo $customer_id_en; ?>";
